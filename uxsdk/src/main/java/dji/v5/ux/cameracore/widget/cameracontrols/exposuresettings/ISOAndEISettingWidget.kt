...
open class ISOAndEISettingWidget @JvmOverloads constructor(
    ...
    override fun onAttachedToWindow() {
        ...
        binding.seekbarIso.apply {
            progress = 0
            enable(false)
            addOnSeekBarChangeListener(this@ISOAndEISettingWidget)
            isBaselineVisibility = false
            setMinValueVisibility(true)
            setMaxValueVisibility(true)
            setMinusVisibility(false)
            setPlusVisibility(false)
        }
        binding.buttonIsoAuto.setOnClickListener(this)

        // EI seekBar
        binding.seekbarEi.apply {
            addOnSeekBarChangeListener(this@ISOAndEISettingWidget)
            visibility = GONE
            setMinValueVisibility(true)
            setMaxValueVisibility(true)

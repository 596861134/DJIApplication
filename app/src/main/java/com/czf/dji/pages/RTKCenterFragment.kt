...
import com.czf.dji.databinding.FragRtkCenterPageBinding
import com.czf.dji.models.RTKCenterVM
...
class RTKCenterFragment : DJIFragment(), CompoundButton.OnCheckedChangeListener, RadioGroup.OnCheckedChangeListener {
    ...
    private lateinit var binding: FragRtkCenterPageBinding
    ...
    private fun showRTKSystemStateInfo(rtkSystemState: RTKSystemState?) {
        rtkSystemState?.run {
            binding.tvRtkHealthyInfo.text = if (rtkHealthy) {
                "healthy"
            } else {
                "unhealthy"
            }
